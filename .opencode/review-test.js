...
(async () => {
  ...
  await page.route('https://api.github.com/**', async route => {
    ...
    if (url.includes('contents/')) {
      // listFiles → 空配列, getFile → content/sha オブジェクト
      if (url.endsWith('/')) {
        return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([]) });
      }
  ...
  const checks = await page.evaluate(() => ({
    heatBodyExists: !!document.getElementById('heatBody'),
    heatPresetsExist: document.querySelectorAll('.heat-preset').length > 0,
    heatCellsExist: document.querySelectorAll('#heatBody .cell').length > 0,
    rangeTabsExist: document.querySelectorAll('.range-tabs button').length > 0,
    trendSvgExists: !!document.getElementById('trendSvg'),
    dowBodyExists: !!document.getElementById('dowBody'),
    weatherBodyExists: !!document.getElementById('weatherBody'),
    bestBodyExists: !!document.getElementById('bestBody'),
    worstBodyExists: !!document.getElementById('worstBody'),
    memoBodyExists: !!document.getElementById('memoBody'),
    heatBodyText: document.getElementById('heatBody')?.textContent?.substring(0, 100) || '',
  }));

  console.log('DOM checks:', JSON.stringify(checks, null, 2));
  console.log('Errors:', errors.length ? errors : 'none');

  await page.screenshot({ path: '/Users/hideakimacbookair/taxi-daily-report/.opencode/review-screenshot.png', fullPage: true });
  await browser.close();
})();
