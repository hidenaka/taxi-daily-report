...
// 営業明細写真の前処理（Node・サーバー側）。生画像Buffer → 向き補正 → 書類検出
// → 条件付き台形補正 → 傾き補正 → グレースケール＋拡大 → OCR投入可能な canvas。
//
// ロジックは ocr-spike/auto-preprocess.mjs（97-98%実証済）と同一。
// 違いはファイル入出力を廃し、Buffer を受けて canvas を返すこと（ディスク非使用）。
...
// グレースケール化し、OCR検出に十分な解像度へ拡大（目標幅3200px）。
// 注: 適応的二値化は PP-OCR 精度を落とすため不採用（グレースケール＋拡大が最良）。
...
/**
 * 生画像Bufferを前処理し、OCR投入可能な canvas を返す（保存しない）。
 * @param {Buffer} imageBuffer
 * @returns {Promise<object>} 前処理済み canvas
 */
